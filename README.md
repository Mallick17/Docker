# 🛠️ Docker Container Management Tasks

## Docker Hands-On Task Breakdown
This section documents a series of practical, hands-on Docker operations that demonstrate how to manage containers, images, and custom environments. The tasks cover:

- Pulling official Docker images like **httpd**, **nginx**, and **amazonlinux** from Docker Hub.
- Running containers with specific configurations (naming containers, exposing ports, using detached mode).
- Stopping, starting, and removing containers and images to manage system resources effectively.
- Creating custom Docker images from running containers using `docker commit`.
- Building Docker images using a **Dockerfile** with predefined installation and setup steps (e.g., installing and configuring Apache Tomcat).
- Pushing custom images to **Docker Hub** and pulling them back.
- Managing a **multi-node Docker Swarm cluster**, including node promotion, scaling services, and deploying replicated services.
- Introduction to Docker Volumes and Docker Compose for persistent storage and multi-container setups (placeholders present for future tasks).

Together, these tasks illustrate real-world container lifecycle management and foundational DevOps practices using Docker.
